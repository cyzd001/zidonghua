...
class jiekouFrame(Frame):
    ...
    def createPage(self):
        ...
        Button(self, text="格式化", width=8).place(x=490, y=530)
    ...
    def button3(self):
        self.datain.delete("0.0", "end")
    def button4(self):
        data002 = self.dataout.get("0.0", "end").strip()
        data003 = data002.replace("null", '"null"').replace("true", '"true"').replace("false", '"false"')
        json_str = json.dumps(eval(data003), sort_keys=True, indent=2, ensure_ascii=False)
        self.dataout.delete("0.0", "end")
        self.dataout.insert("0.0", json_str)
mainpage(jiemian)
jiemian.mainloop()
